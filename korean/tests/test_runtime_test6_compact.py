...
import build_korean_patch_test6 as test6
...
def test_compact_character_info_uses_exactly_two_korean_glyph_pairs() -> None:
    built, report = test6._make_wbase_compact_two_glyphs(_synthetic_wbase())

    # Test5's gender conversion remains a string call; male/female Korean
    # labels are two glyphs. Race/class fields terminate after exactly 4 bytes
    # so the 2-byte Korean encoding can never be cut in half.
    for offset in (0x1FF3, 0x2034, 0x20DC, 0x2112):
    ...
    assert report["gender_compact_korean_glyphs"] == 2


def test_test6_keeps_test5_safe_refresh_wrapper_installed() -> None:
    # Importing build_korean_patch_test6 imports the Test5 release wrapper,
    # which replaces stable.make_wroot with the safe fallback implementation.
