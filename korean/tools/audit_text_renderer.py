#!/usr/bin/env python3
"""Audit the known Wizardry VI DOS text-rendering path before patching.

The tool intentionally does *not* modify WROOT.EXE or EGA.DRV.  It validates the
byte signatures relied on by the Korean renderer design and reports the largest
resident zero-filled code cave in WROOT.

Known retail-build path (WROOT values below are *file offsets*; subtract the
0x200 MZ header for load-module CS offsets):

* WROOT file 24B7 / CS:22B7 - write one character into a window/cell and draw it
* WROOT file 26E9 / CS:24E9 - NUL-terminated WFONT0 string loop
* WROOT file 271D / CS:251D - NUL-terminated WFONT1..4 string loop
* WROOT file 25B0 / CS:23B0 - far call through display-driver entry pointer CS:1B8A
* WROOT file 2E69 / CS:2C69 - refresh/repaint call through the same entry
* EGA.DRV COM:0103 -> renderer at 029A (AL * 8, WFONT0)
* EGA.DRV COM:0107 -> renderer at 053A (AL * 32, WFONT1..4)

The two string loops increment their source pointer by one byte before reading a
character.  That is the principal hook point that must become custom-pair aware.
"""
...
def audit(wroot: bytes, ega: bytes) -> dict[str, object]:
    ...
    signatures = [
        # Function prologues / pointer-walk signatures.
        ...
        (0x271D, bytes.fromhex("5I 8B EC 83 EC 04 56 06 1E"), "WROOT WFONT1-4 string file:271D / CS:251D"),
    ...
    for cs_offset, sig, label in signatures:
        expect(wroot, cs_offset, sig, label, issues)

    # EGA.DRV is a COM image loaded at offset 0x100.  File offsets are COM-0x100.
    ega_signatures = [
